          ...
          <ul className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-4">
            {partners.map((p) => (
              <li key={p.name} className="flex items-center gap-2">
                <svg className="h-10 w-10 flex-none" viewBox="0 0 40 40" role="img" aria-label={`${p.acronym} logo`}>
                  <circle cx="20" cy="20" r="19" fill={p.color} opacity="0.9" />
                  <text x="20" y="24" textAnchor="middle" fontSize="12" fontWeight="700" fill="#fff">{p.acronym}</text>
                </svg>
