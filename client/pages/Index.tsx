import { ArrowRight, Building2, FileText, Map, Shield, Sparkles } from "lucide-react";
  ...
  return (
    ...
    <main className="min-h-[calc(100vh-4rem)] bg-gradient-to-b from-background to-white">
      ...
      <section className="container grid lg:grid-cols-2 gap-8 items-center py-16">
        <div>
          <span className="inline-flex items-center gap-2 rounded-full bg-primary/10 text-primary px-3 py-1 text-xs font-semibold">
            <Sparkles className="h-3.5 w-3.5" /> AI-powered FRA Atlas & WebGIS DSS
          </span>
          ...
          <p className="mt-4 text-base md:text-lg text-muted-foreground max-w-prose">
            A government-grade platform for secure digitization, AI asset mapping, and actionable decision support — designed for Ministries, District Officers, Forest Departments, and NGOs.
          </p>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 text-sm">
            ...
            <Select value={role} onValueChange={(v)=> setRole(v as Role)}>
              <SelectTrigger className="w-48 h-9"><SelectValue placeholder="Select role" /></SelectTrigger>
              <SelectContent>
          ...
          <div className="mt-6 flex items-center gap-4 text-sm text-muted-foreground">
            ...
            <div className="flex items-center gap-2"><Building2 className="h-4 w-4" /> Govt-ready</div>
          </div>
        </div>
        <div className="relative">
          ...
          <div className="absolute -bottom-4 left-6 bg-white rounded-md shadow px-3 py-2 text-xs border">
            <span className="font-semibold">Map Controls:</span> zoom, basemap, measure, draw, export, layers
          </div>
        </div>
      </section>
      ...
      <section className="container py-8 grid md:grid-cols-3 gap-6">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5 text-primary" /> Digitization</CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            OCR-based onboarding for Pattas and documents. Human-in-loop verification ensures accuracy.
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Map className="h-5 w-5 text-primary" /> FRA Atlas</CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            Unified view across State → District → Block → Village. Interactive WebGIS with layers.
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-primary" /> DSS</CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            Scheme recommendations based on rules like water index, land type and coverage thresholds.
          </CardContent>
        </Card>
      </section>
      ...
      <section id="partners" className="mt-4">
        ...
        <div className="container py-10 grid md:grid-cols-2 gap-8">
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">Madhya Pradesh, Tripura, Odisha, Telangana</p>
          </div>
          <div>
