...
export default function History() {
  const { notificationsList, history, markAllNotificationsRead, removeHistory } = useApp();
  const nav = useNavigate();
  const loc = useLocation();
  const params = React.useMemo(()=> new URLSearchParams(loc.search), [loc.search]);
  const initialFilter = (params.get("type") as "all" | "export" | "rejected") || "all";
  const [filter, setFilter] = React.useState<"all" | "export" | "rejected">(initialFilter);

  const filtered = history.filter((h) => {
    if (filter === "all") return true;
    if (filter === "export") return h.type === "export";
    if (filter === "rejected") return /rejected/i.test(h.title) || /rejected/i.test(h.description);
    return true;
  });
  ...
  const setAndSyncFilter = (f: typeof filter) => {
    ...
    if (f === "all") p.delete("type"); else p.set("type", f);
    ...
    <main className="container py-8 grid lg:grid-cols-2 gap-6">
      <div className="lg:col-span-2 -mt-2 mb-1 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-sm">
          <Button size="sm" variant={filter === "all" ? "default" : "outline"} onClick={()=> setAndSyncFilter("all")}>All</Button>
          <Button size="sm" variant={filter === "export" ? "default" : "outline"} onClick={()=> setAndSyncFilter("export")}>Exports</Button>
          <Button size="sm" variant={filter === "rejected" ? "default" : "outline"} onClick={()=> setAndSyncFilter("rejected")}>Rejected</Button>
        </div>
      </div>
