...
export const OcrUploadModal: React.FC<{
  ...
  const { addHistory, addNotification, addClaim, setOcrDraft, selectedState, selectedTribe } = useApp();
...
=======
          ...
          <div className="flex items-center justify-end gap-2">
            <Button variant="destructive" onClick={()=>{ toast({ title: "Claim rejected", description: "Marked as rejected." }); addHistory({ type: "status", title: "Claim Rejected", description: "Claim moved to Rejected." }); onOpenChange(false); }}>Reject</Button>
            <Button variant="outline" onClick={()=>{ setOcrDraft({ pattaId, name, village, coordinates, claimType, status: "Pending" }); nav("/patta"); onOpenChange(false); }}>Edit</Button>
            <Button onClick={()=>{ const id = crypto.randomUUID(); addClaim({ id, pattaId, name, village, coordinates, claimType, status: "Pending", createdAt: new Date().toISOString(), source: "ocr" }); toast({ title: "Claim saved", description: `Saved ${pattaId}` }); addHistory({ type: "upload", title: "OCR Accepted", description: `Saved: ${pattaId}` }); addNotification({ title: "New Claim Uploaded", description: `${pattaId} saved` }); onOpenChange(false); nav(`/patta?id=${id}`); }}>Accept</Button>
>>>>>>> 6548e770c42125b862edafd3fcf9a3601e227221
          </div>
        </div>
      </DialogContent>
