...
export default function DssRecommendations() {
  ...
  const exportReport = (s: typeof recos[number]) => {
    toast({ title: "Report exported", description: `${s.scheme} report generated` });
    addHistory({ type: "export", title: `${s.scheme} Export`, description: `Report generated for ${location.village ?? "selected village"}` });
          ...
          {recos.map((r)=> (
            <div key={r.scheme} className="flex items-start justify-between">
              ...
              <div className="space-x-2">
                <Button variant="outline" onClick={()=> exportReport(r)}>Export</Button>
                <Button onClick={()=> assign(r)} disabled={!!assigned[r.scheme]}>{assigned[r.scheme] ? "Assigned" : "Assign"}</Button>
              </div>
            </div>
          ))}
        </CardContent>
